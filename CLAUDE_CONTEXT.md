...
## SEO-архитектура (2026-07-26)
**Правило canonical:** в `src/app/layout.tsx` НЕТ `alternates`. Next сливает метаданные поверхностно, поэтому `canonical: '/'` в корне наследуется каждой страницей без своего `alternates` — все турниры/команды/игроки указывали canonical на главную и выпадали из индекса. Каждая публичная страница ставит canonical сама через `trilingualAlternates()`/`canonicalFor()` из `src/lib/seo.ts`. НЕ возвращать `alternates` в корневой layout.
**То же с картинками:** `openGraph.images` в метаданных перекрывает файловую конвенцию `opengraph-image.tsx`. В корне и на страницах с генерируемой карточкой `images` не задаём.
- `src/lib/seo.ts` — canonical/hreflang + билдеры JSON-LD (Organization, WebSite+SearchAction, SoftwareApplication, BreadcrumbList, FAQPage, SportsEvent, SportsTeam, Person, SportsOrganization, ItemList), `jsonLdGraph()` собирает один `@graph` на страницу
- `src/lib/entitySeo.ts` — title/description сущностей на 3 языках; `src/lib/sportSeo.ts` — 32 дисциплины (slug + падежи), `src/lib/formats.ts` — названия форматов (используются и визардом создания)
- **Языковые URL:** публичные страницы живут по трём адресам — `/x` (ru), `/kz/x`, `/en/x`. Тела страниц — в `src/components/public/*` (`TournamentPage`, `LeaguePage`, `LeaguePlayersPage`, `TeamPage`, `PlayerPage`, `MatchPage`), роуты — тонкие обёртки, задающие `LANG`. Cookie `lang` НЕ используется для языка публичной страницы: краулер без cookie видел бы только русский. Вместо этого `src/proxy.ts` редиректит посетителя с cookie kz/en на префиксный адрес (`PREFIXABLE` regex — держать в синхроне с реально существующими /kz//en маршрутами)
- Спорт-лендинги `/sports/[sport]` (+ /kz, /en) — 96 статических страниц, ISR 1 ч; хабы `/tournaments`, `/leagues`, `/sports` — ISR 10 мин; читают БД через `src/lib/supabase/public.ts` (без cookie, иначе роут становится динамическим)
- **OG-картинки:** `src/lib/og.tsx` (`renderOgCard`) + `src/lib/ogEntities.ts`, шрифт `assets/Geist-Regular.ttf` (SIL OFL, `assets/Geist-OFL.txt`) — Satori без встроенного шрифта рисует пустые квадраты вместо кириллицы. Файл попадает в serverless-бандл через `outputFileTracingIncludes` в `next.config.ts`
- `src/app/robots.ts` — закрыты /dashboard, /account, /admin, /api, /checkout, /login, /register; `sitemap.ts` отдаёт все языки, лимиты рассчитаны на 50 000 URL с учётом ×3
- IndexNow (Bing/Яндекс): `src/lib/indexnow.ts` + ключ-файл `/indexnow-key.txt`, дёргается при создании турнира/чемпионата. Без `INDEXNOW_KEY` тихо ничего не делает

## Pending Tasks (as of 2026-07-26)
...
0.1 Применить миграцию `046_fixture_played_at.sql` — до этого `played_at` не пишется (`updateFixtureResult` в `src/lib/fixtures.ts` откатывается на запись без колонки), в JSON-LD у матчей нет даты, а `matches_played_7d` считает по старой логике
0.2 SEO-запуск: добавить в Vercel env `GOOGLE_SITE_VERIFICATION`, `YANDEX_VERIFICATION` (токены из Search Console / Вебмастера, выводятся в `<head>` автоматически), `INDEXNOW_KEY` (любая hex-строка 8–128 символов); затем подтвердить домен в обеих панелях и отправить `https://tournable.app/sitemap.xml`

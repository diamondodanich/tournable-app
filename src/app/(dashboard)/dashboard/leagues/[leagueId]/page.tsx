...
export default async function LeagueManagePage({
  ...
  const [{ data: leagueRaw }, { data: seasonsRaw }, { data: teamsRaw }] = await Promise.all([
    ...
    supabase.from('league_teams').select('id, players(count)').eq('league_id', leagueId),
  ])
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const teamsCount = (teamsRaw ?? []).length
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const playersCount = ((teamsRaw ?? []) as any[]).reduce((sum, t) => sum + (t.players?.[0]?.count ?? 0), 0)

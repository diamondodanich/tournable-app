...
export async function generateMetadata({ params }: { params: Promise<{ sport: string }> }): Promise<Metadata> {
  ...
  if (!entry) return { title: 'Бет табылмады', robots: { index: false, follow: false } }
  ...
  return {
    ...
    openGraph: {
      ...
      url: absUrl(`/kz${path}`), siteName: 'Tournable', locale: 'kk_KZ',
    },
    twitter: { card: 'summary_large_image', title: copy.h1, description: copy.description },
  }
}

export default async function SportPageKz({ params }: { params: Promise<{ sport: string }> }) {
  const { sport } = await params
  const entry = sportSeoBySlug(sport)
  if (!entry) notFound()

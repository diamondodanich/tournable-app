...
export default function LiveBoard({ tournament, teams, initialGame, isOwner, defaultHomeId, defaultAwayId }: Props) {
  ...
  // Track started_at separately to avoid stale closure in interval
  const gameRef = useRef<LiveGame | null>(initialGame)
  gameRef.current = game

  // Realtime subscription — for viewers only (owner uses optimistic state)
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        // Viewers always update from DB; owner uses optimistic state
        if (!isOwner) {
          setGame(g)
  ...
  // Timer tick — runs off local state for both owner and viewers
  useEffect(() => {
  ...
  // Direct Supabase update helper (no server action round-trip)
  ...
  async function handleInit() {
    ...
    setIniting(true)
    const newGame: Omit<LiveGame, 'id' | 'created_at'> = {
    ...
    <div className={boardClass}>
      ...
      <div className="flex items-center gap-6 sm:gap-10 mb-6 w-full max-w-lg justify-center">
        ...
        <div className="flex flex-col items-center gap-2 flex-shrink-0">
          <span className={`font-mono font-black text-emerald-400 ${fullscreen ? 'text-4xl' : 'text-2xl'}`}>
      ...
      {/* Tournament name + reset match (owner) */}

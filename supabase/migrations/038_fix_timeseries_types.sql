-- =============================================
-- 038: Фикс типов в metrics_timeseries()
-- =============================================
-- Ошибка: "structure of query does not match function result type".
-- Причина: в Postgres sum(bigint) возвращает NUMERIC, а не bigint. Поэтому
-- выражение `s0 + sum(...) over (...)` для кумулятивных колонок давало numeric
-- при объявленном bigint. Лечится явным приведением каждой возвращаемой
-- колонки к заявленному типу.
...
begin
  ...
  select
    cal.d::date,
    coalesce(s_day.n, 0)::bigint,
    coalesce(t_day.n, 0)::bigint,
    ((select s0 from base) + sum(coalesce(s_day.n, 0)) over (order by cal.d))::bigint,
    ((select t0 from base) + sum(coalesce(t_day.n, 0)) over (order by cal.d))::bigint
  from cal

-- =============================================
-- TOURNABLE — recent_users показывает и внутренние аккаунты
-- =============================================
-- Миграция 036 убрала из списка аккаунты с is_internal — правильно для метрик,
-- где свои и тестовые аккаунты искажают выручку. Но с появлением выдачи планов
-- прямо в таблице (UserPlanCell) список перестал быть аналитикой и стал рабочим
-- инструментом: пока живых регистраций нет, он оказывался пустым целиком, и
-- выдать план было негде.
--
-- Теперь фильтр снят, а флаг отдаётся наружу — таблица помечает такие строки.
-- В product_metrics() фильтр остаётся: там считаются деньги.
--
-- Возвращаемый набор колонок меняется, поэтому функцию нужно удалить: у
-- create or replace нет права менять сигнатуру returns table.

drop function if exists public.recent_users(integer);
